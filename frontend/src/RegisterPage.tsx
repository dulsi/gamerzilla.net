...
import { userRegister, getServerOptions, ServerOptions, defaultServerOptions } from './User';
...
export const RegisterPage: FC = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const serverOpts = await getServerOptions();
        if (!serverOpts.canRegister) {
...
const handleRegisterSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    } else if (lowerMsg.includes('email in use')) {
        ...
        >
          <Flex direction="column" gap="4" align="center" py="4">
            ...
            <Callout.Root color="blue" role="alert">
              ...
              <Callout.Text>
                An activation link has been sent to <strong>{email}</strong>. Please click the link
                in the email to activate your account.
              </Callout.Text>
            </Callout.Root>
            <Text size="2" color="gray" align="center">
              Once verified, you will be able to sign in.
            </Text>
        ...
        <form onSubmit={handleRegisterSubmit}>
          <Flex direction="column" gap="5">
            ...
            <Flex direction="column" gap="4">
              ...
              {/* 3. Always Render Email, but Conditionally Require it */}
              <label>
                <Text as="div" size="2" mb="1" weight="bold">
                  Email {options.emailEnabled && <RequiredMark />}
                </Text>
                <TextField.Root
                  size="3"
                  type="email"
                  
                  placeholder={options.emailEnabled ? 'Required for verification' : 'Optional'}
                  value={email}
