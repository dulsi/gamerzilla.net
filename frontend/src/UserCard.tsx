...
interface UserCardProps {
  ...
  onAction: (userName: string, type: 'approve' | 'delete' | 'promote' | 'demote') => void;
}
            ...
            <Flex gap="2" align="center">
              ...
              {!user.approved && (
                <Tooltip content="Pending Approval">
                  <ShieldAlert size={16} color="var(--amber-9)" />
                </Tooltip>
              )}
            </Flex>
            ...
            <Box onClick={(e) => e.stopPropagation()} style={{ display: 'flex' }}>
              <DropdownMenu.Root>
                ...
                <DropdownMenu.Content align="end">
                  ...
                  {loggedInUser?.admin &&
                    (user.admin ? (
                      ...
                      loggedInUser.userName !== user.userName && (
                        <DropdownMenu.Item
                          color="red"
                          onSelect={() => onAction(user.userName, 'demote')}
                        >
                      ...
                      <DropdownMenu.Item
                        color="amber"
                        onSelect={() => onAction(user.userName, 'promote')}
                      >
                  ...
                  {loggedInUser?.admin && !user.approved && (
                    <DropdownMenu.Item
                      color="green"
                  ...
                  {loggedInUser?.admin && loggedInUser.userName != user.userName && (
                    <DropdownMenu.Item
                      color="red"
                      onSelect={() => onAction(user.userName, 'delete')}
                    >
